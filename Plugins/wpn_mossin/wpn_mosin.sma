...
#define PLUGIN "Mosin Nagant"
#define VERSION "1.0"
#define AUTHOR "KORD_12.7 (Basic Code) And BG Rampo (Optimized)"


// Weapon settings
#define WEAPON_NAME 			"weapon_mosin"
#define WEAPON_SLOT			5
#define WEAPON_POSITION			4
#define WEAPON_PRIMARY_AMMO		"7.62 Russian"
...
#define WEAPON_DAMAGE			500.0

// Hud
#define WEAPON_HUD_TXT			"sprites/weapon_mosin.txt"

// Ammobox
#define AMMOBOX_CLASSNAME		"ammo_mosin"

// Models and sounds
#define MODEL_CLIP			"models/w_mosin_clip.mdl"
#define MODEL_SHELL			"models/shell_762x39.mdl"

#define MODEL_WORLD		"models/w_mosin.mdl"
#define MODEL_VIEW		"models/v_mosin.mdl"
#define MODEL_PLAYER		"models/p_mosin.mdl"
	
#define SOUND_FIRE		"weapons/mosin_shoot1.wav"
...
public plugin_init()
{
	...
	new imosin = wpnmod_register_weapon
	...
	new iAmmomosin= wpnmod_register_ammobox(AMMOBOX_CLASSNAME);
	
	wpnmod_register_weapon_forward(imosin, Fwd_Wpn_Spawn, "mosin_Spawn");
	wpnmod_register_weapon_forward(imosin, Fwd_Wpn_Deploy, "mosin_Deploy");
	wpnmod_register_weapon_forward(imosin, Fwd_Wpn_Idle, "mosin_Idle");
	wpnmod_register_weapon_forward(imosin, Fwd_Wpn_PrimaryAttack, "mosin_PrimaryAttack");
	wpnmod_register_weapon_forward(imosin, Fwd_Wpn_SecondaryAttack, "mosin_SecondaryAttack");
	wpnmod_register_weapon_forward(imosin, Fwd_Wpn_Reload, "mosin_Reload");
	wpnmod_register_weapon_forward(imosin, Fwd_Wpn_Holster, "mosin_Holster");
	
	wpnmod_register_ammobox_forward(iAmmomosin, Fwd_Ammo_Spawn, "Ammomosin_Spawn");
	wpnmod_register_ammobox_forward(iAmmomosin, Fwd_Ammo_AddAmmo, "Ammomosin_AddAmmo");
}
...
public mosin_Spawn(const iItem)
{
	// Setting world model
...
public mosin_Deploy(const iItem, const iPlayer, const iClip)
{
...
public mosin_Holster(const iItem)
{
...
public mosin_Idle(const iItem, const iPlayer, const iClip)
{
...
public mosin_PrimaryAttack(const iItem, const iPlayer, iClip)
{
	...
	wpnmod_set_offset_float(iItem, Offset_flNextPrimaryAttack, 1.88);
	wpnmod_set_offset_float(iItem, Offset_flNextSecondaryAttack, 1.98);
	wpnmod_set_offset_float(iItem, Offset_flTimeWeaponIdle, 7.0);
	
	wpnmod_set_player_anim(iPlayer, PLAYER_ATTACK1);

	wpnmod_send_weapon_anim(iItem, ANIM_FIRE_1);
	...
	vecPunchangle[0] = random_float(-4.0, 4.0);
...
public mosin_SecondaryAttack(const iItem, const iPlayer)
{
	wpnmod_set_think(iItem, "mosin_Stab");
...
public mosin_Stab(const iItem, const iPlayer)
{
	...
	if (flFraction < 1.0)
	{
		...
		wpnmod_set_think(iItem, "mosin_Smack");
		set_pev(iItem, pev_nextthink, get_gametime() + 0.1);
	}
...
public mosin_Smack(const iItem)
{
...
public mosin_Reload(const iItem, const iPlayer, const iClip, const iAmmo)
{
	...
	wpnmod_default_reload(iItem, WEAPON_MAX_CLIP, ANIM_RELOAD_A, 4.35);
}
...
public Ammomosin_Spawn(const iItem)
{
	// Setting world model
...
public Ammomosin_AddAmmo(const iItem, const iPlayer)
{
	new iResult = 
	(
		ExecuteHamB
